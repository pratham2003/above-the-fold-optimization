...
<?php
	...
	if (!isset($this->CTRL->options['update_count']) || intval($this->CTRL->options['update_count']) > 0) {
		...
		<p style="font-size:16px;">
			Without Critical CSS a <a href="https://en.wikipedia.org/wiki/Flash_of_unstyled_content" target="_blank">Flash of Unstyled Content</a> is caused when CSS optimization is enabled and it will trigger the two Google PageSpeed rules <a href="https://developers.google.com/speed/docs/insights/OptimizeCSSDelivery?hl=<?php print $lgcode;?" target="_blank"><em>Eliminate render-blocking JavaScript and CSS in above-the-fold content</em></a> and <a href="https://developers.google.com/speed/docs/insights/PrioritizeVisibleContent?hl=<?php print $lgcode;?" target="_blank"><em>Prioritize visible content</em></a>, reducing the PageSpeed score significantly.', 'abovethefold').'
			</p>
		<p>
